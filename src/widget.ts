...
export class ExcalidrawWidget extends Widget {
  ...
  private _excalidrawAPI: any | null = null;
  ...
  flushToModel(): void {
    ...
    const data = buildExcalidrawData(
      this._excalidrawAPI.getSceneElements() as Record<string, unknown>[],
      this._excalidrawAPI.getAppState() as Record<string, unknown>,
      (this._excalidrawAPI.getFiles() ?? {}) as Record<string, unknown>
    );
  ...
  private _renderReact(): void {
    ...
    const element = React.createElement(Excalidraw, {
      ...
      onChange: this._onExcalidrawChange.bind(this) as any
    });
  ...
  private _onExcalidrawChange(
    ...
    this._debounceTimer = setTimeout(() => {
      ...
      this._model.fromExcalidrawData(
        buildExcalidrawData(elements, appState, files)
      );
      this._isUpdatingModel = false;
    }, CHANGE_DEBOUNCE_MS);
  }
  ...
  private _onModelChanged(): void {
    ...
    if (this._excalidrawAPI) {
      ...
      if (data.files && Object.keys(data.files).length > 0) {
        this._excalidrawAPI.addFiles(Object.values(data.files) as any[]);
      }
    }
  }
...
> {
  constructor(
    options: DocumentWidget.IOptions<ExcalidrawWidget, ExcalidrawModel>
  ) {
    super(options);
